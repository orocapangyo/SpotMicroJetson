...
규칙
1)각 관절의 회전축을 $z$ 축으로 좌표계 선정
2)각 링크에 연결된 관절의 회전축이 서로 직각이 되게 $x$ 축 선정
3)$x_{i-1}$ 축에 대한 회전( $z_i$ 통일)
4)$x_{i-1}$ 축에 대한 이동
5) $z_{i}$ 축에 대한 이동
6) $z_{i}$ 축에 대한 회전( $x_i$ 통일)

![로봇팔](image/week04_1.png)
![로봇팔](image/week04_2.png)

$\theta_i$: zᵢ축을 기준으로 xᵢ에서 xᵢ₊₁까지의 회전 각도 (조인트 회전 각)
$d_i$:  zᵢ축을 따라 xᵢ와 xᵢ₊₁ 사이 거리 (링크 오프셋)
z축의 화살표 방향으로 이동하면 **+**, 반대 방향으로 이동하면 **-**입니다. 
$a_i$: xᵢ축을 따라 zᵢ와 zᵢ₊₁ 사이 거리 (링크 길이) 
항상 양수(+)로 잡는 것이 관례입니다.
$\alpha_i$: xᵢ축을 기준으로 zᵢ에서 zᵢ₊₁까지의 회전 각도 (링크 뒤틀림 각)
